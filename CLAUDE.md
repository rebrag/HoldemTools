...
`/compare`'s builder queues one whenever its `Players` control is 3+ (`frontend/src/pages/compare/multiwaySeats.ts` widens the config it already built rather than teaching `TreeBuilding` about seats, since that panel is shared with the PioSOLVER upload path).
