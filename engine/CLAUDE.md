...
- **The DCFR discount is deferred, and `run()` is the only thing that settles it.** The factors owed for iteration t are paid per node on its first visit in iteration t+1, when the rows are already in cache; `run()` flushes whatever is left so callers read exactly what an eager sweep would have produced. `iterate()` is private for this reason - reading regrets or strategy sums between iterations would see half-discounted state. Anything new that reads them (a pruning pass, QRE) must either run inside the traversal or flush first.
