...
- **Fixed lambda costs 3.7x wall clock against dcfr; ANNEALED lambda is 1.44x FASTER than dcfr to the same Nash target** (one flop tree, one schedule - see `docs/roadmap.md` M7). They are different products of the same code path and must not be conflated: fixed lambda is the modelling feature, annealed lambda is the Nash accelerator. Only the annealed one competes with dcfr.
