...
- **The preflop solver is abstraction-free in the HAND dimension, sampled in the CHANCE dimension, and exact on hero blockers.** That sentence is the whole honest accounting and it belongs wherever the mode is described. Every combo keeps its own regret row, strategy row and EV - the 169 rollup is export-only aggregation, exactly as it already is postflop - so this does not cross the hand-abstraction line below. What IS approximated is the board (a fixed seeded sample, reproducible from `board_sample`) and **opponent-vs-opponent card removal, which is dropped at 3+ seats and exact at 2**. The bunching cost is visible as a chip-conservation error: root EVs on the 4-way 10bb spot sum to 0.068 chips instead of 0. Surfaced as `opponent_card_removal: "hero_only"` in artifact metadata and reported by `tests/test_preflop_game.cpp`.
