...
- **A QRE solve is not Nash and must never be compared to Pio.** The harness (`watcher/engine_compare.py`) refuses; keep it that way. The refusal is scoped to runs that actually involve Pio (`pio_enabled`) - an engine-only extraction of a QRE artifact is fine and is how `/compare` renders one. Entropy-regularized CFR's QRE convergence carries the same "empirical in multiway" caveat as CFR itself.
...
- **Fixed lambda costs 3.7x wall clock against dcfr. ANNEALED lambda reliably cuts ITERATIONS by ~1.35x (1.31-1.41x over a 24-board sweep) but costs 1.44x per iteration, so wall clock is a wash or slightly worse** - see `docs/roadmap.md` M7 for the table. Do not repeat the earlier "1.44x faster" claim; it came from one spot and did not survive the sweep. The per-iteration overhead (one `logf` per regret cell plus a `compat_weights` pass per actor node) is the lever: reduce it and the iteration saving becomes a real win.
