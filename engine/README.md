...
3. **HU NLHE river vs. PioSolver** - unabstracted, so any difference is an engine bug. Manual harness on the machine that has Pio:
   ...
   For eyeballing the per-hand numbers, add `--json-out compare.json` and load the file on the frontend's hidden `/compare` page (no nav entry): side-by-side 13x13 strategy grids with EV-heat mode, per-action summaries and per-combo breakdown panels for both solvers, and a per-combo table (frequencies, EVs, ΔEV, L1) for every node. The file loads from disk in the browser, so the page works on the deployed site too.
   On the dev machine the page can also drive the whole loop itself: its PioViewer-style tree builder (board / pot / stacks / sizes / accuracy stop) posts to the dev-only `POST /api/engine/compare`, which runs htsolver and the `--solve-pio` harness and returns the comparison. Point the frontend at the local API (`frontend/.env.local` -> `http://localhost:5192`) and run the `backend-engine` launch profile.
4. Turn, then small flop trees - after river passes (future milestones).
