...
- `team_joint` (with a team; engines from 2026-09-03) - the exact joint strategy the rollup is marginalized from, keyed by real cards.
  ...
  `nodes[node id]` = `{actor, partner, num_actions, freq, weight, weight_max, ev, ev_scale}`: `freq` is base64 `u16 LE[orbit_count * (num_actions - 1)]` divided by 65535 (the first actions; the last is one minus the rest); `weight` is base64 `u16 LE[orbit_count]`, the row's reach mass relative to `weight_max` (0 = never reached); `ev` is base64 `i16 LE[orbit_count * num_actions]` times `ev_scale` in TEAM chips, with `-32768` meaning no data.
