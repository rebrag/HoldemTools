...
**Heads-up postflop on the sampled core (2026-09-07): a comparison path, not the product path.**
`NlhePostflopGame` now wears `DealGame` too, so `algorithm.family: "sampled"` runs any 3/4/5-card root board and `/compare` can race the two cores to one accuracy target (`sampledConfig` on a compare job; both artifacts stamp `metadata.convergence`).
What it is: uniform dealing from the deck minus the root board, hole cards then the runout in deal order (`Deal.board` carries ONLY the runout - the traversal follows `board[chance_depth]`); strengths read off the evaluator the tree already built for that completed board; showdown values against the pinned opponent; iso refused by the parser (the core reads storage directly and cannot go through the redirect).
What it is not: fast on tight ranges - a dealt hand outside a seat's range weighs zero, so the effective sample size per hero traversal is the OPPONENT's range fraction (measured: a 3%-range flop spot sat at 26% of pot exploitable after 200k iterations, while a full-range river reached 3.5% at 400k).
Two bugs paid for on the way: `ev_walk` matched `deal.board[0]` at every chance level (fine preflop and on a turn tree, threw on a flop tree's river card), and `sampled_ev` weighed every deal 1.0 - correct only for identical unweighted ranges, i.e. every preflop spot so far - so it now weights by the range product, bit-identical on full ranges.
The EV pass on postflop deals seats' hands IN PROPORTION to their ranges with the exact importance weight (`DealGame::sample_ev_deal`), which is legitimate there (every seat pinned, no runout marginal to bias) and is what turned a 92/8 EV split on a 70/30 spot into 69.5/30.5.
