...
TEST_CASE("memory estimate matches the sizing rule") {
  ...
  // nodes x hands x actions x 8 rule.
  CHECK(CfrSolver::state_bytes(game) == 4 * 3 * 2 * 8);

  const MemoryEstimate estimate = estimate_memory(game);
  CHECK(estimate.regret_strategy_bytes == 192);
  CHECK(estimate.tree_bytes == game.tree().size() * sizeof(Node));
  CHECK(estimate.workspace_bytes > 0);
  CHECK(estimate.total() ==
        estimate.regret_strategy_bytes + estimate.tree_bytes + estimate.workspace_bytes);
}
