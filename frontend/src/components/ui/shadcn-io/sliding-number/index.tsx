  ...
  return (
    ...
    >
      {isInView && Number(number) < 0 && <span className="mr-1">-</span>}
        ...
        const isLast = index === intPlaces.length - 1;

        // Add a comma AFTER this digit if there are a multiple of 3 digits to the right
        // e.g. 1,234,567 → comma after the "1" and "4"
        const shouldShowComma =
          !isLast &&
          ((intPlaces.length - index - 1) % 3 === 0);

        return (
          <React.Fragment key={`int-${place}-${index}`}>
            <SlidingNumberRoller
