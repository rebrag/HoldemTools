...
import { displayedPot as sharedDisplayedPot } from "@/lib/pokerPot";
...
// lib/pokerPot's displayedPot - chips only count as "in the pot" once the
// street they were wagered on is complete. While a street is live, each
// player's current-street commitment still sits in front of them as chips
// (see buildTableSeats), so it is excluded to avoid double-counting. When the
// street ends, `advanceStreet` zeroes `committed` (the pot keeps the chips),
// so the collected amount folds into this figure exactly then. At showdown
// the hand is resolved and the whole pot is shown.
export function displayedPot(e: Engine): number {
  if (e.done) return e.pot;
  return sharedDisplayedPot(
    e.pot,
    e.players.map((p) => p.committed)
  );
}
